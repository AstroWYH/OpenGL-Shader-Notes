...
ANC_LOGD("WatermarkRender::drawWatermark cur_fbo:%d", cur_fbo);

// 根据cur_fbo情况，考虑是否生成watermark_fbo_
if (cur_fbo == GL_NONE) {
    if (watermark_fbo_ == GL_NONE) {
        glGenFramebuffers(1, &watermark_fbo_);
        ANC_LOGD("WatermarkRender::drawWatermark gen watermark_fbo_:%d", watermark_fbo_);
    }
    glBindFramebuffer(GL_FRAMEBUFFER, watermark_fbo_);
}
ANC_LOGD("WatermarkRender::drawWatermark watermark_fbo_:%d", watermark_fbo_);
...
glGetFramebufferAttachmentParameteriv(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                                      ...
                                      &fbo_attached_tex);
ANC_LOGD("WatermarkRender::drawWatermark fbo_attached_tex:%d dst_tex:%d", fbo_attached_tex, dst_tex);
